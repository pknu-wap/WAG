...
const JoinUser = forwardRef<HTMLDivElement, JoinUserProps>(
  ({ Nickname, isCaptain, isReady, gameStart, currentCycle, gameUserDto, whoseTurn, className, onClick, children }, ref) => {
    ...
    const getGameAnswer = async () => {
      try {
        const response = await answerApi.getGameAnswerList(roomId ? parseInt(roomId) : 0, myName ? myName : '');
        return response;
      } catch (error) {
    ...
    return (
      <div ref={ref} className={`${className} flex flex-col justify-center items-center relative mx-1`}>
        {Nickname === whoseTurn ? ( // 내 턴일 때 주황색으로 프로필 색깔 바뀌도록
          <PopoverComponent isOpen={isPopoverOpen} onToggle={handleTogglePopover} tooltipChildren={children}>
            <IconButton
              size='lg'
