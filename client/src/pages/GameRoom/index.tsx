...
import ReadyStartButton from '../../Game/RedayStartButton';
...
const GameRoom = () => {
  ...
  const getRoomIdCode = async (enterCode: number) => {
    try {
      const response = await roomApi.getRoomIdByCode(enterCode);
      return response;
    } catch (error) {
      console.error(error);
      throw error;
  ...
  const nicknamePossibleClickRenderButton = () => {
    if (nickname === beforeNickname) {
      return (
        <Button
          className='mt-3 m-auto flex justify-center items-center'
          size='sm'
          disabled={false}
          onClick={nicknamePossibleClick}
        >
          닉네임 확인
        </Button>
      );
  ...
  const getRoomInfo = async () => {
    try {
      const response = await roomApi.getRoomInfo(Number(params.roomId));
  ...
  const ClickReady = () => {
    ...
    handlePlaySound();
    if (isReady) Toast({ message: '준비 취소 ❌', type: 'error' });
    else Toast({ message: '준비 완료 ✅', type: 'success' });
  };
  ...
  const { time, startTimer, stopTimer, resetTimer }: TimerHookProps = useTimer();
  ...
  const handleTimerEnd = () => {
    ...
    if (gameCycleRef.current !== currentCycle) {
      // 사이클 수가 바뀌었다면 게임턴수 재랜더링
  ...
  const getNextTurnInfo = (socketMessage: any) => {
    ...
    currentAnswerRef.current = answerUserDtos.find((user: any) => user.nickname === nextTurnUser.roomNickname);
  };
  ...
  function GameActionButton({ isMyTurn, isAnswerMode }: { isMyTurn: boolean; isAnswerMode: boolean }) {
    ...
    if (isMyTurn) {
      ...
      if (isAnswerMode) {
        return (
          <Button size='sm' className='mr-10' onClick={switchToASK}>
            질문 하기
          </Button>
        );
      } else {
        return (
          <Button size='sm' className='mr-10' onClick={switchToCORRECT}>
            정답 맞추기
          </Button>
        );
      }
  ...
  const LoadingSpinner = () => (
    <>
      ...
      <div className='mt-60'>이름들을 섞는 중이에요!</div>
    </>
  );
          ...
          <div>
            ...
            <div className='m-auto w-3/4'>
              ...
              {gameUserDtos.map((user, index) => {
                return (
                  <RankingUser key={index} roomNickname={user.roomNickname} answer={user.answername} ranking={user.ranking} />
                );
              })}
            </div>
            <div className='m-auto grid-cols-1 md:grid-cols-2 mt-5 gap-2'>
              <Button className='mr-5 mb-5' size='sm' onClick={restartOnClick}>
                재시작하기
              </Button>
              <Button className='mr-5 mb-5' size='sm' onClick={exitOnClick}>
                메인페이지로 가기
              </Button>
            </div>
          </div>
                  ...
                  <textarea
                    ...
                    onKeyDown={(e) => {
                      ...
                      if (e.key === 'Enter' && !e.shiftKey) {
                        // Enter 키를 누를 때 Shift 키가 눌려있지 않은 경우
                ...
                {width > 650 ? (
                  <>
                    <div className='mr-5 mb-5'>
                      <Button size='md' disabled={false} onClick={exitOnClick}>
                        {' '}
                        게임 나가기{' '}
                      </Button>
                    </div>
        ...
        <ReadyToGameModal onRequestClose={closeModal}>
          <div className='flex flex-col justify-between'>
            ...
            <div>
              ...
              {!isNicknameChecked && <div className='text-[#d98e0d]'>닉네임을 검사해주세요!</div>}
            </div>
                  ...
                  <div>
                    ...
                    <div>
                      <DropdownSelect
                        ...
                        defaultValue={count === 1 ? category : selectedOption}
                      />
